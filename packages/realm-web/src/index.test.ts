...
describe("Realm Web", () => {
    ...
    describe("Credentials", () => {
        it("expose a credentials factory", () => {
            expect(typeof Realm.Credentials).to.equal("object");
        });
        ...
        it("expose the anonymous credential", () => {
            ...
            expect(credentials).to.be.instanceOf(Realm.Credentials.AnonymousCredential);
        });

        it("expose the username password credential", () => {
            expect(typeof Realm.Credentials.usernamePassword).to.equal("function");
            const credentials = Realm.Credentials.usernamePassword('gilfoil', 's3cr3t');
            expect(credentials).to.be.instanceOf(Realm.Credentials.UsernamePasswordCredential);
    ...
    describe("static app function", () => {
        it("return the same App instance if ids match", () => {
